...
Proof.
  ...
  injection H1 as Hxz Hj.
  assert (Hyz': y :: l = z :: l).
  { rewrite Hj. apply H2. }
  injection Hyz' as Hyz.
  transitivity z.
  apply Hxz. symmetry. apply Hyz.
  Qed.
(** [] *)
...
Proof.
  ...
  - (* n = S n' *)    
    ...
    + (* m = S m' *)
      simpl in H.
      injection H as H1.
      assert (L: n' + S n' = S n' + n'). { apply add_comm. }
      assert (R: m' + S m' = S m' + m'). { apply add_comm. }
      rewrite L in H1. rewrite R in H1. simpl in H1. injection H1 as H1.
      apply IHn' in H1. 
      f_equal.
      apply H1.
Qed.
...
Proof.
  ...
  - (* l = h l' *)
    ...
    destruct h eqn:LH.
    simpl in H.
    destruct (split l') as [xs ys] eqn: DL'.
    ...
    + (* l1 = h1 l1' *)
      ...
      * (* l2 = [] *)
        discriminate H.
      * (* l2 = h2 l2' *)
        injection H as XH1 XSL1' YH2 YSL2'.
        rewrite <- XH1.
        rewrite <- YH2.
        rewrite -> XSL1' in DL'.
        rewrite -> YSL2' in DL'.
