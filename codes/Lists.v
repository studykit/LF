...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  ...
  | x1 :: xs1 => match l2 with
               | nil => l1
               | x2 :: xs2 => x1 :: x2 :: alternate xs1 xs2
               end
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | nil => []
  | h :: lst => match h =? v with
              | true => lst
              | _ => h :: (remove_one v lst)
              end
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => []
  | h :: lst => match eqb v h with
              | true => remove_all v lst
              | _ => h :: (remove_all v lst)
              end
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  ...
  | nil, _  => true
  | _, nil  => false
  | _ , h2 :: lst2 => subset (remove_one h2 s1) lst2
  end.
...
Proof.
  intros l.
  induction l as [| n l' IHlr'].
  - (* l is [] *)
    reflexivity.
  - (* (n :: l') ++ [] = n :: l' *)
    simpl. rewrite IHlr'. reflexivity. Qed.
