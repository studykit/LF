...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | [] => pair [] []
  | (x, y) :: xs => pair (cons x (fst (split xs))) (cons y (snd (split xs)))
  end.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => andb (leb 7 n) (even n)) l.
...
Lemma map_concat: forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
...
Proof.
    ...
    * (* l = [] *)
      reflexivity.
    * (* l = h :: l' *)
      assert (FR: fold_length (h :: l') = fold (fun _ n => S n) (h :: l') 0).
      { reflexivity. }
      rewrite -> FR. simpl.
